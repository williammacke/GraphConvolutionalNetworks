...
int main() {
	...
	GCNLayer<random_normal_init, relu> layer_i("li", numPapers, 32, 32, relu(),
			random_normal_init(0, 0.01));
	GCNLayer<random_normal_init, softmax> layer2("l2", numPapers, 32, 6, softmax(),
			random_normal_init(0, 0.1), -1);


	Network<cross_entropy_with_logits, adam, GCNLayer<random_normal_init, relu>, GCNLayer<random_normal_init, relu>, GCNLayer<random_normal_init, softmax>> network(numPapers, 6, {}, adam(), handle, sparseHandle, layer1, layer_i, layer2);
	network.setGraph(&g);
